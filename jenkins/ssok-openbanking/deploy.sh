...
BACKEND_IMAGE_NAME="ssok-openbanking"
...
echo
echo "SSOK OPEN-BANKING DEPLOY Process Start......"
echo 
echo $separationPhrase
echo
...
#도커 컴포즈 종료
docker compose -f docker-compose-openbanking.yml down

#도커 이미지 제거
docker image rmi ssok-openbanking:latest
...
#도커 이미지 로드
docker load -i ssok-openbanking.tar

#도커 컴포즈 시작
docker compose -f docker-compose-openbanking.yml up -d
EOT

echo
echo "SSOK OPEN-BANKING DEPLOY Finished!"